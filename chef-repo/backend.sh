...
NODEIP=`for i in $(knife node list | grep BE); do knife node show $i | grep IP: | awk '{print $2}'; done > /root/listip_bend`
NODENAME=`for n in $(knife node list | grep BE); do knife node show $n | grep 'Node Name:' | awk '{print $3}'; done > /root/listnode_bend`
...
do
  ...
  knife bootstrap $f1 --ssh-user chef --ssh-password 'chef@1234' --sudo --use-sudo-password -N $f2 --run-list 'role[backend]' -E production
 done 3</root/listip_bend 4</root/listnode_bend
